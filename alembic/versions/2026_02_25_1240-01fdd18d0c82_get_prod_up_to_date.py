...
def upgrade() -> None:
    ...
    if not has_col('invitation', 'reuseable'):
        with op.batch_alter_table('invitation', schema=None) as batch_op:
            batch_op.add_column(sa.Column('reuseable', sa.Boolean(), nullable=False, server_default=sa.text('0')))
            batch_op.add_column(sa.Column('user_scope', sa.Enum('ADMIN', 'USER', 'DEMO', 'GUEST', name='scope'), nullable=False, server_default='USER'))
    ...
    if not has_col('message', 'skipped_by_condition'):
        with op.batch_alter_table('message', schema=None) as batch_op:
            batch_op.add_column(sa.Column('skipped_by_condition', sa.Boolean(), nullable=False, server_default=sa.text('0')))
